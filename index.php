...
if($button)
 ...
 $cryptKey  = 'qJB0rGtIn5UB1xG03efyCp';
    ...
    $num=mysqli_num_rows($get);
      if(($num==1)&&($get2['status']==1))
     {         
      $namelog=$get2['name'];	  
    $_SESSION['loginname']=$username;
    $_SESSION['name']=$namelog;
...
$ccode=$_POST['ccode'];             
   ...
   $cryptKey  = 'qJB0rGtIn5UB1xG03efyCp';
$input = base64_encode( mcrypt_encrypt( MCRYPT_RIJNDAEL_256, md5( $cryptKey ), $password, MCRYPT_MODE_CBC, md5( md5( $cryptKey ) ) ) );
$sql="INSERT INTO userver (username, password,name,country,mobile) VALUES ( '$username', '$input', '$namesp','$ccode', '$cell')";
...
$message .= '<h1 style="color:#f40;">Hi!!'.$namesp.'</h1>';
$message .= '<p style="color:#f40;">please click here,to activate your account </p>';
$message .= '<a href="http://mrinalini.esy.es/activate.php?account='.$username.'&name='.$namesp.'">clickme</a>';
$message .= '</body></html>';

                        if(mail($to, $subject, $message, $headers)){
   	   		 header("Location:xyz123.php?account=".$username);
				 //header( "refresh:0;url=notactivated.php?account=".$username);
} else{
    header("Location:xyz123.php?account=".$username);
...
#mymodal1body.modal-body{
  ...
  div#wb_element_instance2.wb_element{
	  left:10px;
  }
  div#wb_element_instance5.wb_element{
	  left:280px;
	  top:5px;
  }
...
<div id="wb_element_instance31" class="wb_element" style="text-align: center; width: 100%;"><div class="wb_footer"></div><script type="text/javascript">
  ...
  <div class="modal fade" id="myModal1" role="dialog">
    <div class="modal-dialog">
      ...
      <input type="text" name="name1" class="form-control" id="name" required placeholder="Enter name">
    </div>
<div class="form-group">
    ...
    <div class="form-group" id="pass">
      ...
      <input type="password" name="password1" class="form-control" id="password" required placeholder="must contain at least 6 characters ">
    </div><br>
	<div class="form-group" id="confpass">
      <label for="pwd"  >Confirm Password:</label>
      <input type="password" name="password2" class="form-control" id="password1" required placeholder="must contain at least 6 characters " >
    </div>
